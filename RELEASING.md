...
The `main` branch carries the released 5.1.1 contract plus unreleased
...
The 5.1.1 release tag is exactly `5.1.1`; do not create or document a `v5.1.1`
...
```bash
INNOFLOW_REQUIRE_RELEASE_TAG=1 INNOFLOW_RELEASE_VERSION=5.1.1 scripts/check-release-sync.sh
```

That command intentionally requires the actual local tag name to be `5.1.1`.
It must not normalize or accept `v5.1.1`.
